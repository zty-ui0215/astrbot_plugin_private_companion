...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：本插件已完成重构，被动回复注入减少 2/3，实现缓存命中从 1/10 优化至 4/5。后续持续人格维护 + 体验打磨阶段。
...
插件还提供可选的“每日 Token 软限额”（默认 `800,000`）。它是“达到限额就停止插件/停止后台链路”的替代方案：达到软限额后，插件会优先保留用户当下触发的回复、图片转述和合并转发处理，暂缓新闻整理、网页探索、创作、群聊片段/黑话整理、主动消息润色、关系分析、夹层视觉和主动生图等低优先级后台任务。
