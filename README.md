...
- 重要：更新5.0.0前请确保备份Astrbot/config/插件配置！！！如果更新后丢失配置请先不要进行保存，配置没丢但如果保存覆盖就是另一回事了（
...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.0.0 已完成主动框架重置，主动消息从单一定时管线升级为调度、候选语义、判定、生成复核、执行回写的分层架构。后续进入持续人格维护 + 体验打磨阶段。
...
- 休息回复闸门：默认关闭。开启后，当前日程处于睡眠、午休或休息段时，普通被动回复会先经过闸门；可选择“按概率醒来”或“由模型判断是否该醒来”，模型模式会按重要性、是否明确叫醒、情绪/安全需要打分，达到醒来阈值才回复。用户明确说不要打扰会静默，紧急或明确叫醒会优先放行。
...
6. 如果只想让 Bot 偶尔主动来找你，不希望它参与普通被动回复，可以开启“主动消息专用模式”。开启后普通私聊/群聊注入、图片/转发处理、防抖、群聊观察、TTS 后处理等会被运行模式覆盖；关闭后原配置会恢复生效。
7. 如果保留普通被动增强，建议让“被动注入位置”使用默认的尾部/缓存友好策略。它会把变化较大的状态片段放到请求后部，减少破坏主模型缓存前缀；只有确实需要更强约束时再改成系统提示词注入。
...
- 相关设置：主动图片分享、生图后端、文生图工作流、自拍工作流、本地图片读取保护。
- 电脑高负荷时可延后本地 ComfyUI/SDGen 生图；“自动后端”模式下会依次尝试 ComfyUI、SDGen、在线图片 API。

生图后端可选值：

- 自动：依次尝试 ComfyUI、SDGen、在线图片 API。
...
- 外部图片 API：只使用 OpenAI 兼容的在线图片 API。
...
- 内置设置：QQ 空间联动、生活说说发布、生活说说最小间隔、生活说说概率、说说配图生成、带图说说概率。
