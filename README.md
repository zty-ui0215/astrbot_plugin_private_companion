...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：现工作重心转向直播插件，本插件已转入功能维护状态；有好的想法也可以联系我，QQ：995051631，欢迎提交 Issues，目标是无愧“最强”之名，喜欢的话请点一个 Star。

## 3.6.0 整合更新

本版本把近期围绕直播联动和 TTS 链路的细碎补丁统一收束为一个稳定版本。TTS 生成后的本机播放和直播打字机同步现在会区分来源：只有直播自动回应语音会触发本机播放和直播字幕，普通私聊、群聊或其他窗口的语音不会再串到直播 overlay，也不会在直播状态下从本机扬声器播放。

TTS 后处理新增了可见字幕文本和实际朗读文本的分离能力。直播场景中，中文回复可作为 QQ/打字机显示文本，日语或其他语种朗读稿只用于语音合成，避免把朗读稿作为可见消息或字幕重复输出。Windows 本机静默播放也继续保留 WAV 头修复和后台播放逻辑。
