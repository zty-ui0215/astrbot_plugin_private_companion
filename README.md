...
本插件的定位不是单纯的主动私聊插件，也不是单独的记忆插件。它更像 AstrBot 之上的“陪伴编排层”：负责把人格、日程、状态、私聊关系、群聊上下文、长期记忆、外部动作和模型 Provider 组织到同一个连续体验里。

核心目标是“连续存在感”。Bot 会有自己的当天状态、生活节奏、长期小计划、关系认知和群聊观察，而不是只在用户发消息时临时拼一段回复。
...
- 世界观适配：可把现代能力转译成当前人设能理解的世界内说法，例如异世界冒险者的公会闲谈、行囊书匣、水晶映像或旅途安排。
- 模型与成本编排：为日程、细化、日记与梦境、创作、回复自检、关系分析、记忆整理、生图提示词等任务分别指定 Provider。
- 扩展页管理：在 AstrBot WebUI 中查看私聊、群聊、关系网、状态、梦境、书柜、额度、主动计划、Token 消耗和模块配置。
...
- `enable_environment_perception`：启用时间、日期和平台环境感知。
- `max_daily_messages`：每个用户每日主动消息上限。
- `idle_minutes`：用户空闲多久后才允许主动。
- `min_interval_minutes`：两次主动之间的最小间隔。
- `quiet_hours`：免打扰时间。
- `worldview_adaptation_mode`：世界观适配模式，默认 `auto`，可手动设为 `fantasy`、`sci_fi`、`modern`、`custom` 或 `off`。
- `worldview_adaptation_prompt`：自定义现代能力到人设世界观的转译提示。
...
建议使用火山引擎火山方舟“协作计划”的免费模型额度覆盖日常成本。按当前插件调用结构估算，在私聊、群聊观察、日程、梦境、记忆整理、关系网、自登记和 Token 监控等功能全开的情况下，通常每天消耗约 `300K-500K tokens`。
...
- 本插件内置配置：`enable_environment_perception`、`environment_perception_timezone`、`enable_holiday_perception`、`holiday_country`、`enable_platform_perception`、`enable_lunar_perception`、`enable_solar_term_perception`、`enable_almanac_perception`
- 自动避让：检测到 `astrbot_plugin_llmperception` 或 `astrbot_plugin_LLMPerception` 时，会自动关闭 `enable_environment_perception`。

### 群聊场景感知

- 参考插件：`astrbot_plugin_context_aware`
- 仓库：<https://github.com/muyouzhi6/astrbot_plugin_context_aware>
- 本插件内置配置：`enable_group_scene_awareness`、`group_scene_recent_limit`
- 自动避让：检测到 `astrbot_plugin_context_aware` 时，会自动关闭 `enable_group_scene_awareness`。
...
- 查看各类 LLM 任务的 Token 用量、调用次数、失败记录和成本分布。
- 配置分项模型 Provider，例如日程生成、日程细化、日记与梦境、私下创作、主动语音文案、昨日对话摘要、回复自检、关系分析、记忆整理、群聊黑话释义和生图提示词模型。
...
- 当前发送时间、时段和工作日/休息日语境。
- 可选农历、临近节气和轻量宜忌氛围。
- 平台、私聊/群聊、群号/群名和消息媒介类型。

这些内容只作为语境边界，不要求 Bot 主动报时或解释平台来源。缺少 `chinese-calendar`、`lunarcalendar` 等可选依赖时，对应字段会自动跳过。

### 2.5 世界观适配层

插件不会假设 Bot 一定生活在现代校园或现实城市。`worldview_adaptation_mode` 会把现实插件能力映射成世界内等价说法：

- `auto`：根据日程专用人设、世界观提示和自定义适配内容粗略判断。
- `fantasy`：群聊可理解为酒馆/公会闲谈，书柜可理解为行囊书匣，识屏可理解为水晶映像，刷视频可理解为翻看吟游诗人的影像记录。
- `sci_fi`：群聊可理解为频道通信，书柜可理解为私人资料柜，识屏可理解为终端画面，刷视频可理解为浏览影像流。
- `custom`：使用 `worldview_adaptation_prompt` 自行描述映射关系。
- `off`：不注入世界观适配提示。

这层只影响表达和上下文理解，不改变真实插件能力。比如“B 站联动”仍然由外部视频插件执行，但最终主动消息会尽量服从当前人格和世界观，而不是直接暴露现代插件词。
...
跨群转述与 @ 群友工具也会优先使用关系网。用户说“帮我发到某群并 @ 某人”时，工具会先用关系节点的 QQ 号确认身份；只有允许回退时，才会继续匹配群昵称或群名片。查询群成员时，已登记成员的昵称和群名片会沉淀到 `observed_names`，方便后续改名后仍能识别。
