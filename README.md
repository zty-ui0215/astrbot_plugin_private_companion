...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：本插件已完成重构，被动回复注入减少 2/3，实现缓存命中从 1/10 优化至 4/5。若无特别影响使用的bug，该插件接下来将着重于主动消息链路的完全重构，目标是更加拟人、贴近设定。
...
- 模型与成本编排：为日程、细化、日记与梦境、创作、新闻整理、主动搜索、回复/主动复核、关系分析、记忆整理、合并消息转述、群聊判断等任务分别指定 Provider。
