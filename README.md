...
- 配置边界：插件里的 `VOICE_PROMPT_PROVIDER_ID` 和 `tts_conversion_provider_id` 都是文本模型，只负责生成/修正语音文案和 `<tts>` 标记；真正把文本合成音频的是 AstrBot 当前会话的 TTS provider。阿里云百炼/CosyVoice 等语音合成模型需要在 AstrBot 的 TTS 配置中启用，本插件会读取当前会话的 TTS provider。
